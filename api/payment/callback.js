export default function handler(req, res) {
  ...
  if (!reference) {
    const failureUrl = `${process.env.CLIENT_URL || 'https://lumiprettycollection.com'}/payment/callback?status=failed&message=no_reference`;
    console.warn(`Received callback without reference. Redirecting to: ${failureUrl}`);
    return res.redirect(302, failureUrl);
  }

  const successUrl = `${process.env.CLIENT_URL || 'https://lumiprettycollection.com'}/payment/callback?reference=${reference}`;
